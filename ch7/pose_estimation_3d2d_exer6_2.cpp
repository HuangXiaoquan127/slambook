...
void bundleAdjustment (
    ...
    const vector< Point3f >& pts1_2nd,
    const vector< Point2f >& pts2_1st,
    const Mat& K,
    Mat& R, Mat& t );
...
public:
    ...
    virtual void computeError()
    {
        ...
        _error = v2->estimate().inverse().toMinimalVector() - v1->estimate().toMinimalVector();
    }
...
int main ( int argc, char** argv )
{
    ...
    vector<Point3f> pts_3d, pts2_3d;
    vector<Point2f> pts_2d, pts1_2d;
    for ( DMatch m:matches )
    {
        ...
        d = d2.ptr<unsigned short> (int ( keypoints_2[m.trainIdx].pt.y )) [ int ( keypoints_2[m.trainIdx].pt.x ) ];
        if ( d == 0 )   // bad depth
            continue;
        dd = d / 5000.0;
        p1 = pixel2cam ( keypoints_2[m.trainIdx].pt, K );
        pts2_3d.push_back ( Point3f ( p1.x*dd, p1.y*dd, dd ) );
        pts1_2d.push_back ( keypoints_1[m.queryIdx].pt );
    }

    cout<<"3d-2d pairs: "<<pts_3d.size() <<endl;
    cout<<"group2 3d-2d pairs: "<<pts2_3d.size() <<endl;
    ...
    bundleAdjustment ( pts_3d, pts_2d, pts2_3d, pts1_2d, K, R, t );
}
    ...
    Mat& R, Mat& t )
{
    ...
    //points id = (2 + 76) -- (78 + 76)    
    for ( const Point3f p:pts1_2nd )   // landmarks
    {
        g2o::VertexSBAPointXYZ* point = new g2o::VertexSBAPointXYZ();
        point->setId ( index++ );
        point->setEstimate ( Eigen::Vector3d ( p.x, p.y, p.z ) );
    ...
    EdgeSE32SE3* edge = new EdgeSE32SE3();
    edge->setId( index );
    //edge->setVertex( 0, dynamic_cast<g2o::VertexSE3Expmap*> (pose) );
    edge->setVertex( 0, dynamic_cast<g2o::VertexSE3Expmap*> (pose) );
    edge->setVertex( 1, dynamic_cast<g2o::VertexSE3Expmap*> (pose_1st) );
    ...
    cout<<"T1="<<endl<<Eigen::Isometry3d ( pose_1st->estimate() ).matrix() <<endl;
    cout<<"T1="<<endl<<Eigen::Isometry3d ( pose_1st->estimate() ).rotation().inverse() <<endl;
}
